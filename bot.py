...
async def insta_downloader(_, message):
    ran = await message.reply_text("<code>Instergram is not available yet. It will available soon...</code>")
    #link = message.text
    #user_id = message.from_user.id
    ...
    #can_download, remaining_time = can_download_video(user_id, is_premium)
        ...
        #if remaining_time is not None:
            #minutes = remaining_time // 60
            #await ran.edit_text(f"<code>Sorry, you need to wait {minutes} minutes  before downloading another video.</code>")
        #else:
            #await ran.edit_text("<code>Sorry, you have reached your maximum download limit.</code>")
        #return
        
    #url = "https://snapinsta.tools/action.php"
...
async def download_yt(client, message):
    ...
    with ytd.YoutubeDL(options) as ytdl:
        try:
            await msg.edit_text("<code>Downloading Video.....</code>")
            ytdl.download([yurl])
            result = ytdl.extract_info("{}".format(yurl))
    ...
    for file in os.listdir('.'):
        if file.endswith(".mp4"):
            await msg.edit_text("<code>Uploading Video.....\n\nsometimes this take some time🙂 Upload speed depend on video size</code>")
            await app.send_chat_action(message.from_user.id, enums.ChatAction.UPLOAD_VIDEO)
            await message.reply_video(f"{file}", caption=f"**Here Is your Requested Video👆**\n\n<code>{title}</code>\n\n🔗 Requestor : ||{message.from_user.mention}||\n🚀 Downloaded via: [All Save Bot ☘️](https://t.me/AIl_Save_Bot)", reply_markup=InlineKeyboardMarkup(SHARE_BUTTON))
            os.remove(f"{file}")
            break
        else:
